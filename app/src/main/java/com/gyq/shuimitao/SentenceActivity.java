...
public class SentenceActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(MainActivity.mode==1)
            setTitle("Shuimitao（test）");
        else
            setTitle("Shuimitao");
        mListView = (ListView) findViewById(R.id.slistView);
        mTextView = (TextView) findViewById(R.id.sspell);
        ...
        process();
        mNext = (TextView) findViewById(R.id.stv_forword);
        ...
        mInfo = (TextView) findViewById(R.id.stv_info);
        mInfo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                if(id.equals(""))
                    return;
                List<String> list1 = SentenceLearningInfo(id);
                String info = "";
                for(int i=0;i<list1.size();i++)
                    info = info + "\n" + list1.get(i);
                new Gongju().ShowMsg(SentenceActivity.this,
                        spell+" 学习情况",
                        info,false,-1);
            }
        });
    ...
    public void process(){
        ...
        mDataList.remove(0);
        mTextView.setText(spell.replace(chinese,"___"));
        if(spell.length()>=128)
            mTextView.setTextSize(15+5);
        else if(spell.length()>=64)
            mTextView.setTextSize(20+5);
        else if(spell.length()>=32)
            mTextView.setTextSize(30+5);
        else if(spell.length()>=16)
            mTextView.setTextSize(40+5);
        else if(spell.length()>=8)
            mTextView.setTextSize(60+5);
        else
            mTextView.setTextSize(80+5);
        int START = 0;
        int END = 3;
    ...
    public List<String> getWord(){
        ...
        try {
            String sel = "select id,sens,spell,lf,ltn from sentence where lf=? and ltn <?";
            String[] where ={"2",t1};
        ...
        try {
            int START1 = 1;
            int END1 = 1434;
            String[] fn = {"sens", "spell"};
            Random random = new Random();
            while(info.size()<5) {
                int n1 = random.nextInt(END1 - START1 + 1) + START1;
                c1 = mydb.tselect("sentence",""+n1, fn);
                while (c1.moveToNext()) {
                    if (info.size() < 1) {
                        id = ""+n1;
    ...
    public List<String> SentenceLearningInfo(String id1){
        mydb =new DatabaseHelper(this);
        List<String> info = new ArrayList<String>();
        String[] fn = {"lf","lt1","lt2","lt3","lt4",
                "lt5","lt6","lt7","lt8"};
        Cursor c1 = mydb.tselect("sentence",id1,fn);
        try {
            while (c1.moveToNext()) {
    ...
    public void UpdateDbSLearning(String test_time,
                                 String is_success){
        mydb =new DatabaseHelper(this);
        String[] fn = {"lf","lt1","lt2","lt3","lt4",
                "lt5","lt6","lt7","lt8"};
        Cursor c1 = mydb.tselect("sentence",id,fn);
        try {
            while (c1.moveToNext()) {
                for(int i=0;i<fn.length;i++){
                    if(i==0){
                        int info1 = c1.getInt(i);
                        if(info1==0) {
                            mydb.update("sentence",id,fn[i],is_success);
                            mydb.update("sentence",id,fn[i+1],test_time);
                            String nlt = new Gongju().NextLearningTime(test_time,0,0,
                                    0,0,5);
                            mydb.update("sentence",id,"ltn",nlt);
                            break;
                        }
                    }else{
                        String lt = LastSLearningTime();
                        if(lt.contains(":")) {
                            String lt1 = lt.split(":")[0];
                            int lt2 = Integer.parseInt(lt.split(":")[1]);
                            int k = UpdateSIndex(lt1, test_time, lt2);
                            if(k != lt2) {
                                mydb.update("sentence",id, fn[k], test_time);
                                String nlt = "";
                                if(k==2)
    ...
    public void InsertDbSStatics(String id,String test_time,
                                String is_success){
        mydb =new DatabaseHelper(this);
        String[] fn = {"id","answer","dt"};
        String[] fv = {id,is_success,test_time};
        try {
            mydb.insert("sstatics",fn,fv);
        }catch (Exception e){
            Toast.makeText(this,e.toString(),Toast.LENGTH_LONG).show();
        }
    }

    public String LastSLearningTime(){
        mydb =new DatabaseHelper(this);
    ...
    public int UpdateSIndex(String last_time,String cur_time,
                           int last_index) {
/*
第一个记忆周期：5分钟
第二个记忆周期：30分钟
...
//                            Toast.LENGTH_LONG).show();
                    ...
                    UpdateDbSLearning(t1,"1");
                    InsertDbSStatics(id,t1,"1");
                }
                else {
//                    Toast.makeText(context,
//                            "答错了，要继续努力哦！",
//                            Toast.LENGTH_LONG).show();
                    ...
                    UpdateDbSLearning(t1,"2");
                    InsertDbSStatics(id,t1,"2");
                }
                checkAll();
                checkReverse();
